import { CommandHandler } from '@nestjs/cqrs'
import type { TxClient } from '@ceicavs/db'
import { defineAbilityFor, Action, Subject } from '@ceicavs/shared'
import { BaseCommandHandler } from '../../../../common/cqrs'
import type { IDomainEvent } from '../../../../common/cqrs'
import { IDatabaseService } from '../../../../common/database/database.abstract'
import { IEventEmitter } from '../../../../common/events/event-emitter.abstract'
import { ForbiddenException, NotFoundException } from '../../../../common/errors'
import { IRecordingRepository } from '../../interfaces/recording.repository'
import { DeleteRecordingCommand } from './delete-recording.command'

@CommandHandler(DeleteRecordingCommand)
export class DeleteRecordingHandler extends BaseCommandHandler<DeleteRecordingCommand, void> {
  constructor(
    protected readonly db: IDatabaseService,
    protected readonly eventEmitter: IEventEmitter,
    private readonly recordingRepository: IRecordingRepository,
  ) {
    super(db, eventEmitter)
  }

  protected async handle(
    command: DeleteRecordingCommand,
    tx: TxClient,
    _events: IDomainEvent[],
  ): Promise<void> {
    const ability = defineAbilityFor(command.role)

    if (!ability.can(Action.DELETE, Subject.RECORDING)) {
      throw new ForbiddenException()
    }

    const recording = await this.recordingRepository.findById(command.id, tx)

    if (!recording) {
      throw new NotFoundException('Recording')
    }

    if (recording.userId !== command.userId) {
      throw new ForbiddenException()
    }

    await this.recordingRepository.softDelete(command.id, tx)
  }
}
